...
Each solution folder's README has a **problem index** (topic, difficulty, and whether a regression test exists).

## Tests

Regression tests use Node's built-in test runner (no dependencies, Node 18+). Each solution file is loaded unchanged into a `node:vm` context:
